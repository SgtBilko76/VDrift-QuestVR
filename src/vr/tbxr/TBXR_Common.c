...
static void EglInitExtensions()
{
	...
	typedef const GLubyte* (*PFNGLGETSTRINGRAW)(GLenum);
	PFNGLGETSTRINGRAW rawGetString = (PFNGLGETSTRINGRAW)eglGetProcAddress("glGetString");
	const char * allExtensions = rawGetString ? (const char *)rawGetString( GL_EXTENSIONS ) : NULL;
...
static bool ovrFramebuffer_Create(
    ...
    for (uint32_t i = 0; i < frameBuffer->TextureSwapChainLength; i++) {
        ...
        if (glRenderbufferStorageMultisampleEXT != NULL &&
            glFramebufferTexture2DMultisampleEXT != NULL && multisamples > 1) {
            ...
            // Plain framebuffer (no multisampling).
            GL(glGenRenderbuffers(1, &frameBuffer->DepthBuffers[i]));
            GL(glBindRenderbuffer(GL_RENDERBUFFER, frameBuffer->DepthBuffers[i]));
            ...
            GL(glBindFramebuffer(GL_FRAMEBUFFER, 0));
            if (renderFramebufferStatus != GL_FRAMEBUFFER_COMPLETE) {
                ALOGE("Incomplete frame buffer object (plain): %s", GlFrameBufferStatusString(renderFramebufferStatus));
                return false;
            }
        }
    }
...
void ovrFramebuffer_Resolve(ovrFramebuffer* frameBuffer) {
    ...
    // The multisample resolve happens implicitly (EXT_multisampled_render_to_texture).
}
...
void ovrRenderer_Clear(ovrRenderer* renderer) {
	...
	ovrFramebuffer_Clear(&renderer->UiBuffer);
}
...
void ovrRenderer_Create(
	...
	// The 2D layer (menus, HUD). Not multisampled: it is 2D quads and
	// distance-field text, and it is composited as its own layer.
	int uiW = 1280, uiH = 960;
	VR_GetUiSize(&uiW, &uiH);
	ovrFramebuffer_Create(session, &renderer->UiBuffer, GL_SRGB8_ALPHA8, uiW, uiH, 1);
}
...
void ovrRenderer_Destroy(ovrRenderer* renderer) {
	...
	ovrFramebuffer_Destroy(&renderer->UiBuffer);
}
...
void TBXR_InitRenderer(  ) {
    ...
    if (strstr(gAppState.OpenXRHMD, "meta") != NULL)
    {
        ...
        // Get the supported display refresh rates for the system.
        {
            ...
            if (REFRESH > 0) {
                // Requested rate from the game's settings (vr.cfg); the runtime clamps to a supported one.
                OXR(gAppState.pfnRequestDisplayRefreshRate(gAppState.Session, (float)REFRESH));
                gAppState.currentDisplayRefreshRate = (float)REFRESH;
                ALOGV("Requesting display refresh rate %d Hz", REFRESH);
            } else {
...
void TBXR_InitialiseOpenXR()
{
	...
	strcpy(appInfo.applicationName, "VDriftVR");
...
/* Frame submission (xrEndFrame with the projection + UI layers) is in vr_stereo.cpp: VrSubmitFrame(). */
