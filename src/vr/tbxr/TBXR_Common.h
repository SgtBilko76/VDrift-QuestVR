...
typedef struct
{
    ...
    ovrFramebuffer	UiBuffer;      /* the 2D layer: menus and HUD, shown on a quad/cylinder layer */
} ovrRenderer;
...
void VR_FrameSetup();
/* Size of the 2D (menu/HUD) swapchain, asked once when the renderer is created. */
...
/* Acquire/bind/release helpers for any framebuffer (eye or UI). */
